...
def test_construct_jwks_uri():
    ...
    jwt_verifier = BaseJWTVerifier("https://test_issuer.com/oauth2/", "test_client_id")
    actual = jwt_verifier._construct_jwks_uri()
    expected = "https://test_issuer.com/oauth2/v1/keys"
    assert actual == expected
...
async def test_get_jwk(mocker):
    ...
    jwt_verifier = BaseJWTVerifier(
        "https://test_issuer.com", "test_client_id", request_executor=request_executor
    )
...
def test_verify_signature(mocker):
    ...
    mocker.patch(
        "okta_jwt_verifier.jwt_utils.jws._verify_signature", mock_sign_verifier
    )
    ...
    mock_sign_verifier.assert_called_with(
...
async def test_access_token_verifier(monkeypatch, mocker):
    ...
    monkeypatch.setattr(
        BaseJWTVerifier, "verify_access_token", mock_verify_access_token
    )
    issuer = "https://test_issuer.com"
    jwt_verifier = AccessTokenVerifier(issuer)
    await jwt_verifier.verify("test_token")
    mock_verify_access_token.assert_called_with(
        mock_verify_access_token, "test_token", ("iss", "aud", "exp")
    )
...
async def test_id_token_verifier(monkeypatch, mocker):
    ...
    await jwt_verifier.verify("test_token")
    mock_verify_id_token.assert_called_with(
        mock_verify_id_token, "test_token", ("iss", "exp"), None
    )
